...
class NeuralNetworkDepthEstimationNode(Node):
    def __init__(self):
        ...
        self.cre_model = None
        self.frame_counter = 0  # Initialize a frame counter

        # Video writer initialization
        self.video_writer_hitnet = None
        self.video_writer_cre = None
    ...
    def start_rosbag_playback(self):
        try:
            ...
            self.bag_process = subprocess.Popen(['ros2', 'bag', 'play', '/workspaces/depth-estimation-project/Neural_Networks_Depth/src/recordings/indoor_recording/indoor_recording_0.db3'])
            self.get_logger().info('ROS bag playback started.')
        except Exception as e:
            self.get_logger().error(f'Failed to start ROS bag playback: {e}')

    def left_image_callback(self, msg):
        new_left_image = self.bridge.imgmsg_to_cv2(msg, 'mono8')
        if not np.array_equal(new_left_image, self.left_image):
            self.left_image = new_left_image
            self.get_logger().info('Received new left image.')
            self.check_and_initialize_models(self.left_image)
        else:
            self.get_logger().info('Duplicate left image received, skipping...')

    def right_image_callback(self, msg):
        new_right_image = self.bridge.imgmsg_to_cv2(msg, 'mono8')
        if not np.array_equal(new_right_image, self.right_image):
            self.right_image = new_right_image
            self.get_logger().info('Received new right image.')
            self.check_and_initialize_models(self.right_image)
        else:
            self.get_logger().info('Duplicate right image received, skipping...')
    ...
    def initialize_video_writers(self, image_shape):
        fourcc = cv2.VideoWriter.fourcc(*'mp4v')
        self.video_writer_hitnet = cv2.VideoWriter('hitnet_depth_video.mp4', fourcc, 60, image_shape)
        self.video_writer_cre = cv2.VideoWriter('cre_depth_video.mp4', fourcc, 60, image_shape)
    ...
    def process_images(self):
        ...
        self.get_logger().info(f'Starting depth estimation for frame {self.frame_counter}...')

        if self.hitnet_model is None or self.cre_model is None:
            self.get_logger().error("Models not initialized.")
            return

        # Initialize video writers with the frame size
        if self.video_writer_hitnet is None or self.video_writer_cre is None:
            self.initialize_video_writers((self.left_image.shape[1], self.left_image.shape[0]))
    ...
    def publish_depth(self, depth_map, model_type):
        ...
        # Save the depth map frame to video
        if model_type == "HITNET":
            self.publisher_hitnet_depth.publish(depth_msg)
            if self.video_writer_hitnet is not None:
                self.video_writer_hitnet.write(depth_map_colored)
        elif model_type == "CRE":
            self.publisher_cre_depth.publish(depth_msg)
            if self.video_writer_cre is not None:
                self.video_writer_cre.write(depth_map_colored)
    ...
    def publish_stored_depth_maps(self):
        ...
        # Release the video writers after publishing all frames
        self.video_writer_hitnet.release()
        self.video_writer_cre.release()
    ...
    def check_rosbag_completion(self):
        if self.bag_process.poll() is not None:
            self.get_logger().info('ROS bag playback completed.')
            self.exit_flag = True  # Set the exit flag to stop processing

    def listen_for_input(self):
        while not self.exit_flag:
            user_input = input("Press 's' to stop, 'q' to quit: ")
            if user_input.lower() in ['s', 'q']:
                self.get_logger().info(f"Received input '{user_input}'. Stopping process...")
                self.exit_flag = True
                break
    ...
    def main_loop(self):
        while rclpy.ok() and not self.exit_flag:
            ...
            self.process_images()
            self.check_rosbag_completion()
        
        self.get_logger().info('Processing completed. Now publishing stored depth maps...')
        self.publish_stored_depth_maps()
        # self.run_depth_evaluation()
...
def main(args=None):
    ...
    try:
        node.main_loop()  # Run the main loop with frame processing
