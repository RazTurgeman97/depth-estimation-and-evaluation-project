...
class DepthEvaluationNode(Node):
    def __init__(self):
        ...
        self.bagfile_path = '/mnt/data/recordings_merged/indoor_recording_0.db3'
    ...
    # def compute_epe(self, predicted, ground_truth, mask): # irrelevant and should be a vector
    ...
    def print_averages(self):
        ...
        if best_mae == best_rmse == best_mse == best_iou == best_d1: # == best_epe:
    ...
    def analyze_error_localization(self, frame_idx):
        ...
        with open(f"{self.motherdir}/summary_report.txt", "a") as report_file:
            report_file.write(f"\nFrame {frame_idx+1} - Error Localization Analysis:\n")
    ...
    def analyze_model_disagreement(self, frame_idx):
        ...
        with open(f"{self.motherdir}/summary_report.txt", "a") as report_file:
            report_file.write(f"\nFrame {frame_idx+1} - Model Disagreement Analysis:\n")
